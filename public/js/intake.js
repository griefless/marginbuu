...
async function parseBOQViaServer(file) {
  ...
  return normaliseParsedItems(await resp.json());
}
  ...
  return new Promise((res, rej) => {
    ...
    reader.onload = ev => {
      try {
        ...
        const items = [];
        const meta  = { rfq: null, title: null };   // FIX: was undefined, causing ReferenceError
        let lastCat = 'Unclassified';
        for (let r = 0; r < rows.length; r++) {
          const row = rows[r].map(c => String(c === null || c === undefined ? '' : c).trim());
          const fullLow = row.join(' ').toLowerCase();
          if (!meta.rfq) { const m = row.join(' ').match(/RFQ[\s\/\w]+/i); if (m) meta.rfq = m[0].trim().substring(0, 40); }
          if (!meta.title && fullLow.includes('project')) meta.title = row.filter(Boolean).join(' ').substring(0, 80);

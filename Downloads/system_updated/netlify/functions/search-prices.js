...
exports.handler = async function handler(event) {
  ...
  if (!SERPER_KEY) {
    return respond(500, responseHeaders, {
      error: 'SERPER_API_KEY environment variable is not set. Get a free key at https://serper.dev',
    });
  }
...
function isSectionLabel(line) {
  ...
  const hasHeadingKeyword = HEADING_KEYWORDS.some(kw => t.toUpperCase().includes(kw));
  const isAllCaps         = (t === t.toUpperCase() && /^[A-Z\s]+$/.test(t));
  ...
  for (const item of rawItems_pre) {
    ...
    // ── Tabular-path items (XLSX / CSV) ──────────────────────────────────
      ...
      if (isNoiseLine(descCell)) {
        // discard row
      } else if (isSectionLabel(descCell)) {
...
function detectColumnStructure(rows) {
  ...
  const scanRows = Math.min(5, rows.length);
...
function extractFromFreeText(text, source) {
  ...
  // Detects item starts (dotted number prefix) and terminal rows (end with qty+unit).
  // Merges continuation lines into a single logical entry before pattern matching.
  const ITEM_START         = /^[A-Z0-9]+\.[A-Z0-9]/i;
...
async function flaresolverrFetch(url, flareHost) {
  ...
  try {
    const res = await fetch(`${flareHost}/v1`, {
      ...
      body:    JSON.stringify({ cmd: 'request.get', url, maxTimeout: FLARE_TIMEOUT_MS - 5000 }),
      signal:  controller.signal,
    });
    clearTimeout(timer);
    if (!res.ok) return null;
